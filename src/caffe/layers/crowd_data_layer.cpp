...
void CrowdDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  downsamp_times_ = this->layer_param_.crowd_data_param().downsamp_times();
  base_sigma_ = this->layer_param_.crowd_data_param().base_sigma();
  const bool is_color  = this->layer_param_.crowd_data_param().is_color();
  string root_folder = this->layer_param_.crowd_data_param().root_folder();
  ...
  const string& source = this->layer_param_.crowd_data_param().source();
  ...
  if (this->layer_param_.crowd_data_param().shuffle()) {
...
void CrowdDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  CrowdDataParameter crowd_data_param = this->layer_param_.crowd_data_param();
  const bool is_color = crowd_data_param.is_color();
  const bool mirror = this->transform_param_.mirror();
  string root_folder = crowd_data_param.root_folder();
  const unsigned int prefetch_rng_seed = caffe_rng_rand();
  prefetch_rng_.reset(new Caffe::RNG(prefetch_rng_seed));
